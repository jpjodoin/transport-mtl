 ...
 * {@link EulaManager#show(android.app.Activity)} in the onCreate() method of
 * the first activity. If the user accepts the EULA, it will never be shown
 * again. If the user refuses, {@link android.app.Activity#finish()} is invoked
 * on your activity.
 */
public class EulaManager
{
	...
	public static boolean show(final Activity activity)
	{
		...
		final SharedPreferences preferences = activity.getSharedPreferences(PREFERENCES_EULA, Context.MODE_PRIVATE);
		// preferences.edit().putBoolean(PREFERENCE_EULA_ACCEPTED,
		// false).commit(); //FOR TEST TO REMOVE
	...
	private static CharSequence readEula(Activity activity)
	{
		...
		try
		{
			in = new BufferedReader(new InputStreamReader(activity.getAssets().open(activity.getResources().getString(R.string.eula_nomfichier)), "8859_1"), 16384); // 8octet/caracter
																																										// *2048
																																										// =
																																										// 16384
			String line;
